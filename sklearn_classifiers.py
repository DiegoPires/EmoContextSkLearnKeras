...
class SkLearnClassifier(Classifier):
    def __init__(self, data_dto, pre_traitement_dto):
        self.data_dto = data_dto
        self.pre_traitement_dto = pre_traitement_dto # used for prediction, need to apply the same stuff from the training
    ...
    def __create_pipeline(self, classifierTest):
        ...
        self.pipeline.fit(self.data_dto.data_train, self.data_dto.target_train)  

    def __predict(self):
        self.predicted = self.pipeline.predict(self.data_dto.data_test)

    def __mean(self):
        self.accuracy = np.mean(self.predicted == self.data_dto.target_test)

    def show_analyses(self):
        print(metrics.classification_report(self.data_dto.target_test, self.predicted, target_names=self.data_dto.target_names))
    ...
    def predict(self, text):
        if self.pre_traitement_dto.replace_keywords:
            text = replace_keywords_in_text(text)

        if self.pre_traitement_dto.remove_punctuation:
            text = replace_punctuation_in_text(text)

        if self.pre_traitement_dto.apply_lemm:
            text = lemmatize(text, self.pre_traitement_dto)
